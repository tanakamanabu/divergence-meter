...
ダイナミック点灯では各スロットで **2 桁が同時点灯**（グループ A・B から 1 桁ずつ、74141×2）するため、HV 側は常時 2 桁ぶんの電流が流れる。約 5mA（2.5mA×2、マーキング電流込みで最大 7mA、桁切替の重なりを見込んで最大 10mA）。
...
| 電流検出 Rsense | **0.1Ω** / 0.5〜1W（低インダクタンス品） | ピーク電流制限を約 1A に。**V+〜CS 間（ハイサイド）**に入れる |
| FB 分圧 | R1 = **1.8MΩ** : R2 = **15kΩ** | 出力 ≒ 181.5V。R2 は半固定込みで校正 |
| REF バイパス | 0.1µF | REF（pin4）→ AGND。開放不可 |
| ブリーダ抵抗 Rb | 470kΩ / 2W | 電源 OFF 後の放電用（**感電防止・必須**） |

> **MAX1771 のピン接続メモ**（8 ピン、未使用ピン無し）:
> `CS` はハイサイド電流検出入力。**Rsense は V+ と CS の間**に入れ、**インダクタは CS 側**に接続
> （ソース側ではない）。MOSFET ソースは直接 GND。`REF`(pin4) は 0.1µF で AGND へ。
> `SHDN`(pin8) は**アクティブ High**＝常時動作させるなら **GND** に接続。`AGND`/`GND` は 1 点接続。
...
> MAX1771 はハイサイド検出のため、Rsense はインダクタ電流が流れる **V+〜CS 間**に置く。
> `V(V+) − V(CS)` が 100mV に達するとピーク電流制限がかかる。
...
- [ ] 基板設計（桁配置・ドット割り当て）— 回路図入力は [回路図入力ガイド](docs/schematic-guide.md) 参照
